...
def get_client(db: Session, cliente_id: int):
    return db.query(models.Clients).filter(models.Clients.id == cliente_id).first()

def update_client(db: Session, client_id: int, client= schemas.ClientCreate):
    db_cliente = get_client(db, client_id)
    if not db_cliente:
        raise HTTPException(status_code=400, detail="Cliente não enconterado.")
